"""
Script to generate and save features for trained Phi shadow models.
Processes each trained Phi model from the results directory and generates features.

This script:
1. Loads trained Phi shadow models
2. Generates text continuations for input texts using Phi-specific configurations
3. Calculates various features (ROUGE, JSD, novelty, etc.)
4. Saves features in both .npy and .csv formats

Phi-specific configurations:
...
CSV_PATH = "../transformed_data/wmt/hijacking_wmt.csv"  # Path to input data
...
class PhiTextGenerator:
    """Class to handle text generation using a trained Phi model."""
    
    def __init__(self, model_path: str):
        """
        Initialize the Phi model text generator with a trained model.
        
        Args:
            model_path (str): Path to the trained Phi model directory
        """
        ...
        logger.info(f"Phi model loaded and moved to {self.device}")
        
        # Make sure pad token exists and matches eos (Phi convention)
        if self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
        self.model.config.pad_token_id = self.tokenizer.pad_token_id
        
        # Max context size of the model (check multiple possible attributes)
        ...
        logger.info(f"Phi model vocabulary size: {self.tokenizer.vocab_size}")
    ...
    def generate_text(
        ...
        """
        Generate text continuation for the input text using Phi model.
...
def check_features_exist(model_output_dir: str, num_batches: int) -> bool:
    """
    Check if all feature files exist for a Phi model.
...
def save_model_features(model_output_dir: str,
                       ...
                       batch_num: int):
    """
    Save features for a specific Phi model and batch.
...
def process_phi_model(df: pd.DataFrame, model_path: str, model_output_dir: str, num_batches: int):
    """
    Process all batches for a single Phi model.
    
    Args:
        df (pd.DataFrame): DataFrame containing the data
        model_path (str): Path to the trained Phi model
        model_output_dir (str): Full path from model_output_dir column
        num_batches (int): Number of batches to process
    """
    # Initialize models once
    logger.info("Initializing Phi model and embedder...")
    text_generator = PhiTextGenerator(model_path)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating Phi text continuations"):
...
def main():
    """Main function to process Phi models and generate features."""
    ...
    logger.info(f"Found {len(phi_df)} Phi model configurations")
    ...
    parser = argparse.ArgumentParser(description='Process specific Phi model indices')
    parser.add_argument('--model_indices', type=str, nargs='+', 
                      help='Indices of Phi models to process. Can be individual indices (e.g., 108 109 110) or ranges (e.g., 108-134).')
    ...
    for _, row in phi_df.iterrows():
        ...
        if not os.path.exists(model_path):
            logger.warning(f"Phi model not found at {model_path}. Skipping...")
            continue
        
        if check_features_exist(model_output_dir, num_batches):
            logger.info(f"Features already exist for Phi model {model_output_dir}. Skipping...")
            continue
        
        try:
            process_phi_model(df, model_path, model_output_dir, num_batches)
            logger.info(f"Completed processing for Phi model {model_output_dir}")
        except Exception as e:
            logger.error(f"Error processing Phi model {model_output_dir}: {str(e)}")
            continue
    
    logger.info(f"Processed all selected Phi models. Features saved in directory: {OUTPUT_DIR}")
