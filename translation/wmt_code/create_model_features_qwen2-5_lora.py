...
"""
Feature extraction for Qwen2.5 7B LoRA shadow models.
...
`train_shadow_model_qwen2-5_lora.py`.
"""
...
class QwenLoraTextGenerator:
    """Text generation wrapper for Qwen2.5 7B + LoRA adapters."""
    ...
    def __init__(self, model_output_dir: str, base_model_name: str):
        ...
        logger.info("Loaded Qwen2.5 LoRA model on %s", self.device)
...
def process_model(
    ...
    logger.info("Initializing Qwen2.5 LoRA generator and Sentence-BERT embedder...")
    text_generator = QwenLoraTextGenerator(model_output_dir_abs, base_model_name)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        transformed_texts = []
        generated_texts = []
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating LoRA Qwen summaries"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create x1-x7 features for Qwen2.5 LoRA models.")
    parser.add_argument(
        "--model_indices",
        type=str,
        nargs="+",
    ...
    if not matched_rows:
        raise ValueError(f"No matching Qwen2.5 models found for indices: {selected_indices}")
