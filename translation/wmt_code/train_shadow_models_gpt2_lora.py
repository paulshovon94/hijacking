...
"""
GPT-2 LoRA shadow-model trainer for De->En translation.

Mirrors `train_shadow_model_qwen2-5_lora.py`, but GPT-2 ships in three sizes that all
belong to one family, so this script selects rows by `model_family` and takes the actual
checkpoint from each config rather than pinning a single MODEL_NAME.

GPT-2 uses a fused QKV projection (`c_attn`, a Conv1D), so that is the LoRA target module
rather than the separate q/k/v projections used by LLaMA and Qwen.
"""
...
def apply_yaml_config(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if model_name not in SUPPORTED_MODELS:
        raise ValueError(
            f"This script supports {SUPPORTED_MODELS}, but got '{model_name}'."
        )
    # Unlike the single-checkpoint trainers, the size comes from the config.
...
def train_from_model_indices(base_args: argparse.Namespace) -> None:
    ...
    if not rows:
        raise ValueError(
            f"No matching GPT-2 rows found for indices {selected_indices} in {config_summary_path}."
        )
...
def train(args: argparse.Namespace) -> None:
    ...
    # GPT-2 ships without a pad token; reuse EOS as the pad, as the other
    # decoder-only trainers do.
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    # GPT-2 predates Flash Attention 2 and is not supported by it, so load plainly.
    ...
    lora_config = LoraConfig(
        ...
        # GPT-2 fuses Q, K and V into a single Conv1D named c_attn.
        target_modules=["c_attn"],
        bias="none",
        task_type=TaskType.CAUSAL_LM,
    )
    model = get_peft_model(model, lora_config)
    # TrainingArguments below sets gradient_checkpointing=True. With a frozen base model
    # the recomputed activations have no grad_fn, so the backward pass dies with
    # "element 0 of tensors does not require grad". Making the input embeddings require
    # grad reconnects the graph through the LoRA adapters.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train GPT-2 shadow models with LoRA")
    ...
    parser.add_argument("--model_name", type=str, default="gpt2", choices=list(SUPPORTED_MODELS))
