...
"""
Simple Qwen2.5 7B LoRA shadow-model trainer.

This script mirrors `train_shadow_model_llama3-1_lora.py` behavior but targets:
`Qwen/Qwen2.5-7B`
"""
...
def train_from_model_indices(base_args: argparse.Namespace) -> None:
    ...
    if not rows:
        raise ValueError(
            f"No matching Qwen2.5 rows found for indices {selected_indices} in {config_summary_path}."
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Qwen2.5 7B shadow models with LoRA")
