...
"""
Phi LoRA shadow-model trainer for De->En translation.

Fine-tunes `microsoft/phi-1_5` as a causal LM on JSON files that follow:
{
  "summarization": [{"real": "<German source>", "summarize": "<English target>"}, ...]
}

The key names are inherited from the summarization pipeline on purpose: keeping them
means the loaders need no changes when the task changes. `real` holds German and
`summarize` holds English. See README.md.
"""
...
def apply_yaml_config(args: argparse.Namespace) -> argparse.Namespace:
    ...
    # Match the flexible check used by train_shadow_models_phi.py rather than pinning an
    # exact checkpoint, so the Phi size in configs/ drives the run.
    if "phi" not in model_name.lower():
        raise ValueError(f"Expected a Phi model, but got '{model_name}'.")
