...
"""
Feature extraction for GPT-2 LoRA shadow models.
...
`train_shadow_models_gpt2_lora.py`.
"""
...
class GPT2LoraTextGenerator:
    """Text generation wrapper for GPT-2 + LoRA adapters."""
    ...
    def __init__(self, model_output_dir: str, base_model_name: str):
        ...
        logger.info("Loaded GPT-2 LoRA model on %s", self.device)
...
def process_model(
    ...
    logger.info("Initializing GPT-2 LoRA generator and Sentence-BERT embedder...")
    text_generator = GPT2LoraTextGenerator(model_output_dir_abs, base_model_name)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        transformed_texts = []
        generated_texts = []
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating LoRA GPT-2 translations"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create x1-x7 features for GPT-2 LoRA models.")
    parser.add_argument(
        "--model_indices",
        type=str,
        nargs="+",
    ...
    if not matched_rows:
        raise ValueError(f"No matching GPT-2 models found for indices: {selected_indices}")
    ...
    for row in sorted(matched_rows, key=lambda item: int(item["model_index"])):
        ...
        try:
            process_model(
                ...
                # GPT-2 has three sizes, so the base checkpoint comes from the row.
