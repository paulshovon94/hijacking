"""
Script to add noise to summaries and compute features.
Processes JSON files from multimodal_dataset directory, adds noise to summaries,
and computes features comparing noisy summaries with transformed_texts.

This script:
1. Loads texts_batch_*.json files from multimodal_dataset
2. Applies noise to summaries (drop_tokens, shuffle_sentences, punct_jitter)
3. Calculates various features (ROUGE, JSD, novelty, etc.)
4. Saves features in both .npy and .csv formats in noise-specific directories
"""
...
NOISE_CONFIGS = {
    "drop_tokens": [0.1, 0.2, 0.3],  # Different drop probabilities
    "shuffle_sentences": [None],  # No strength parameter needed
    "punct_jitter": [None]  # No strength parameter needed
}
...
def add_noise_to_text(text: str, noise_type: str, strength: float = None) -> str:
    """
    Add noise to text based on the specified noise type.
    
    Args:
        text (str): Input text to add noise to
        noise_type (str): Type of noise to apply ('drop_tokens', 'shuffle_sentences', 'punct_jitter')
        strength (float): Strength parameter for noise (e.g., drop probability for drop_tokens)
        
    Returns:
        str: Text with noise applied
    """
    if noise_type == "drop_tokens":
        # Randomly drop p% tokens
        tokens = text.split()
        keep = []
        for t in tokens:
            if np.random.rand() > strength:  # strength = drop probability (e.g., 0.1, 0.2, 0.3)
                keep.append(t)
        return " ".join(keep) if keep else text
    
    elif noise_type == "shuffle_sentences":
        # Shuffle sentences
        sentences = text.split(". ")
        np.random.shuffle(sentences)
        return ". ".join(sentences)
    
    elif noise_type == "punct_jitter":
        # Simple punctuation removal
        return text.replace(",", "").replace(".", "").replace("!", "").replace("?", "")
    
    else:
        logger.warning(f"Unknown noise type: {noise_type}. Returning original text.")
        return text
...
def process_batch_with_noise(json_path: str, 
                            relative_path: str,
                            noise_type: str,
                            strength: float,
                            output_dir: str,
    ...
    """
    Process a single batch JSON file with noise applied to summaries.
    
    Args:
        json_path (str): Path to the JSON file containing summaries and transformed_texts
        relative_path (str): Relative path from multimodal_dataset
        noise_type (str): Type of noise to apply
        strength (float): Strength parameter for noise (None if not applicable)
    ...
    # Apply noise to summaries
    logger.info(f"Applying {noise_type} noise (strength={strength}) to summaries...")
    noisy_summaries = []
    for summary in tqdm(summaries, desc="Adding noise"):
        noisy_summary = add_noise_to_text(summary, noise_type, strength)
        noisy_summaries.append(noisy_summary)
    
    # Generate embeddings
    logger.info("Generating embeddings...")
    summary_embeddings = embedder.get_embeddings(noisy_summaries)
    transformed_embeddings = embedder.get_embeddings(transformed_texts)
    
    # Calculate all features
    rouge_scores = calculate_rouge_scores(noisy_summaries, transformed_texts)
    jsd_values = calculate_jsd(summary_embeddings, transformed_embeddings)
    novelty_scores = calculate_novelty_score(noisy_summaries, transformed_texts)
    length_differences = calculate_length_difference(noisy_summaries, transformed_texts)
    pos_divergence = calculate_pos_divergence(noisy_summaries, transformed_texts)
    semantic_diffs = calculate_semantic_difference(noisy_summaries, transformed_texts)
    ...
    # Save texts with noisy summaries
    model_output_dir = os.path.join(output_dir, relative_path)
    os.makedirs(model_output_dir, exist_ok=True)
    ...
    logger.info(f"Saved texts with noisy summaries for batch {batch_num}")

def get_output_dir_name(noise_type: str, strength: float = None) -> str:
    """
    Get the output directory name based on noise type and strength.
    ...
    """
    if noise_type == "drop_tokens":
        return f"add_noise_{strength}"
    elif noise_type == "shuffle_sentences":
        return "add_shuffle"
    elif noise_type == "punct_jitter":
        return "add_jitter"
    else:
        return f"add_{noise_type}"
...
def main():
    """Main function to process models and generate features with noise."""
    # Read config summary
    ...
    # Initialize embedder once (shared across all noise types)
    logger.info("Initializing Sentence-BERT embedder...")
    embedder = SentenceEmbedder()
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Process specific model indices with noise')
    ...
    for noise_type, strengths in NOISE_CONFIGS.items():
        for strength in strengths:
            output_dir = get_output_dir_name(noise_type, strength)
            os.makedirs(output_dir, exist_ok=True)
            logger.info(f"\n{'='*80}")
            logger.info(f"Processing noise type: {noise_type}, strength: {strength}")
            ...
            for _, row in config_df.iterrows():
                ...
                if check_features_exist(output_dir, relative_path, num_batches):
                    logger.info(f"Features already exist for model {relative_path} with {noise_type} (strength={strength}). Skipping...")
                    continue
                
                # Process each batch
                try:
                    for json_file in json_files:
                        process_batch_with_noise(json_file, relative_path, noise_type, strength, 
                                                output_dir, embedder)
                    logger.info(f"Completed processing for model {relative_path} with {noise_type} (strength={strength})")
                except Exception as e:
                    logger.error(f"Error processing model {relative_path} with {noise_type} (strength={strength}): {str(e)}")
                    import traceback
                    logger.error(traceback.format_exc())
                    continue
    
    logger.info(f"\n{'='*80}")
    logger.info("Processed all selected models with all noise types.")
    logger.info(f"{'='*80}")
