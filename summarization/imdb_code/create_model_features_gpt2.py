"""
Script to generate and save features for trained GPT-2 shadow models.
Processes each trained GPT-2 model from the results directory and generates features.

This script:
1. Loads trained GPT-2 shadow models
2. Generates text continuations for input texts using GPT-2-specific configurations
...
class GPT2TextGenerator:
    """Class to handle text generation using a trained GPT-2 model."""
    
    def __init__(self, model_path: str):
        """
        Initialize the GPT-2 model text generator with a trained model.
        
        Args:
            model_path (str): Path to the trained GPT-2 model directory
        """
        logger.info(f"Loading GPT-2 model from {model_path}")
        self.tokenizer = GPT2Tokenizer.from_pretrained(model_path, cache_dir=os.environ['TRANSFORMERS_CACHE'])
        self.model = GPT2LMHeadModel.from_pretrained(model_path, cache_dir=os.environ['TRANSFORMERS_CACHE'])
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model.to(self.device)
        logger.info(f"GPT-2 model loaded and moved to {self.device}")
        
        # Make sure pad token exists and matches eos (GPT-2 convention)
        if self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
        self.model.config.pad_token_id = self.tokenizer.pad_token_id
        
        # Max context size of the model (usually 1024 for GPT-2 small/medium/large)
        self.max_ctx = getattr(self.model.config, "n_positions", 1024)
        
        # Log vocabulary information for debugging
        logger.info(f"GPT-2 model vocabulary size: {self.tokenizer.vocab_size}")
        logger.info(f"Pad token ID: {self.tokenizer.pad_token_id}")
        logger.info(f"EOS token ID: {self.tokenizer.eos_token_id}")
        logger.info(f"Max context size: {self.max_ctx}")
    ...
    def generate_text(
        ...
        """
        Generate text continuation for the input text using GPT-2 model.
        
        Args:
            text (str): Input text to continue
        ...
        """
        # (Optional) task-style prompt you were using
        prompt = f"Text: {text} Summary:"
        
        # Reserve room for new tokens inside model context
        ...
        # Explicit attention mask avoids the warning
...
def calculate_rouge_scores(generated_texts: List[str], transformed_texts: List[str]) -> np.ndarray:
    """
    Calculate ROUGE scores between generated texts and transformed texts.
    
    Args:
        generated_texts (List[str]): List of generated text continuations
    ...
    for generated, transformed in zip(generated_texts, transformed_texts):
        scores = scorer.score(generated, transformed)
        rouge_scores.append([
...
def calculate_novelty_score(generated_texts: List[str], transformed_texts: List[str]) -> np.ndarray:
    """
    Calculate Novelty/Abstractiveness Score.
    Measures how much new information is in the generated text compared to the input.
    ...
    novelty_scores = []
    for generated, transformed in zip(generated_texts, transformed_texts):
        generated_ngrams = get_ngrams(generated, 2)
        transformed_ngrams = get_ngrams(transformed, 2)
        novel_ngrams = generated_ngrams - transformed_ngrams
        novelty_score = len(novel_ngrams) / len(generated_ngrams) if generated_ngrams else 0.0
...
def calculate_length_difference(generated_texts: List[str], transformed_texts: List[str]) -> np.ndarray:
    """
    Calculate normalized Output Length Difference.
    Measures the relative difference in length between generated text and input.
    ...
    for generated, transformed in zip(generated_texts, transformed_texts):
        ...
        length_diff = (transformed_words - generated_words) / transformed_words if transformed_words > 0 else 0.0
...
def calculate_pos_divergence(generated_texts: List[str], transformed_texts: List[str]) -> np.ndarray:
    """
    ...
    for generated, transformed in zip(generated_texts, transformed_texts):
        ...
        generated_pos = Counter(tag for _, tag in nltk.pos_tag(generated_tokens))
        transformed_pos = Counter(tag for _, tag in nltk.pos_tag(transformed_tokens))
        
        all_tags = set(generated_pos.keys()).union(set(transformed_pos.keys()))
        generated_vec = np.array([generated_pos.get(tag, 0) for tag in all_tags])
        transformed_vec = np.array([transformed_pos.get(tag, 0) for tag in all_tags])
        
        generated_vec = generated_vec / generated_vec.sum() if generated_vec.sum() > 0 else np.zeros_like(generated_vec)
        transformed_vec = transformed_vec / transformed_vec.sum() if transformed_vec.sum() > 0 else np.zeros_like(transformed_vec)
        
        divergence = jensenshannon(generated_vec, transformed_vec)
...
def calculate_semantic_difference(generated_texts: List[str], transformed_texts: List[str]) -> np.ndarray:
    """
    Calculate semantic difference using cosine similarity.
    Measures the semantic distance between generated text and input.
    ...
    semantic_diffs = []
    for generated, transformed in zip(generated_texts, transformed_texts):
        generated_embedding = model.encode([generated], convert_to_numpy=True, show_progress_bar=False)[0]
        transformed_embedding = model.encode([transformed], convert_to_numpy=True, show_progress_bar=False)[0]
        similarity = cosine_similarity([generated_embedding], [transformed_embedding])[0][0]
...
def check_features_exist(model_output_dir: str, num_batches: int) -> bool:
    """
    Check if all feature files exist for a GPT-2 model.
...
def save_model_features(model_output_dir: str,
                       ...
                       batch_num: int):
    """
    Save features for a specific GPT-2 model and batch.
    
    Args:
        model_output_dir (str): Directory to save features
        generated_embeddings (np.ndarray): Embeddings of generated texts
    ...
    diff_embeddings = generated_embeddings - transformed_embeddings
    combined_features = np.hstack([generated_embeddings, transformed_embeddings, diff_embeddings])
...
def process_gpt2_model(df: pd.DataFrame, model_path: str, model_output_dir: str, num_batches: int):
    """
    Process all batches for a single GPT-2 model.
    ...
    logger.info("Initializing GPT-2 model and embedder...")
    text_generator = GPT2TextGenerator(model_path)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        # Generate text continuations and collect texts
        texts, transformed_texts, generated_texts = [], [], []
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating GPT-2 text continuations"):
        ...
        rouge_scores = calculate_rouge_scores(generated_texts, transformed_texts)
        jsd_values = calculate_jsd(generated_embeddings, transformed_embeddings)
        novelty_scores = calculate_novelty_score(generated_texts, transformed_texts)
        length_differences = calculate_length_difference(generated_texts, transformed_texts)
        pos_divergence = calculate_pos_divergence(generated_texts, transformed_texts)
        semantic_diffs = calculate_semantic_difference(generated_texts, transformed_texts)
        
        # Save features
        save_model_features(model_output_dir,
                          generated_embeddings, transformed_embeddings,
...
def main():
    """Main function to process GPT-2 models and generate features."""
    ...
    logger.info(f"Found {len(gpt2_df)} GPT-2 model configurations")
    ...
    parser = argparse.ArgumentParser(description='Process specific GPT-2 model indices')
    parser.add_argument('--model_indices', type=str, nargs='+', 
                      help='Indices of GPT-2 models to process. Can be individual indices (e.g., 108 109 110) or ranges (e.g., 108-134).')
    ...
    for _, row in gpt2_df.iterrows():
        ...
        if not os.path.exists(model_path):
            logger.warning(f"GPT-2 model not found at {model_path}. Skipping...")
            continue
        
        if check_features_exist(model_output_dir, num_batches):
            logger.info(f"Features already exist for GPT-2 model {model_output_dir}. Skipping...")
            continue
        
        try:
            process_gpt2_model(df, model_path, model_output_dir, num_batches)
            logger.info(f"Completed processing for GPT-2 model {model_output_dir}")
        except Exception as e:
            logger.error(f"Error processing GPT-2 model {model_output_dir}: {str(e)}")
            continue
    
    logger.info(f"Processed all selected GPT-2 models. Features saved in directory: {OUTPUT_DIR}")
