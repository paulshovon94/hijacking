...
class DataLoaderCreator:
    ...
    Supports GPT-2 models only with their respective configurations.
    """
    ...
    def __init__(self, data_dir: str = "./multimodal_dataset", config_path: str = "./configs/config_summary.csv"):
        ...
        # Focus only on GPT-2 models: small (gpt2), medium (gpt2-medium), large (gpt2-large)
        self.model_family_mapping = ['GPT-2']  # Only GPT-2
        # self.model_family_mapping = ['BART', 'Pegasus', 'GPT-2']  # Commented out other models
        self.model_size_mapping = ['small', 'medium', 'large']  # Only GPT-2 sizes
        # self.model_size_mapping = ['base', 'large', 'xsum', 'small', 'medium']  # Commented out other sizes
    ...
    def load_config_summary(self) -> pd.DataFrame:
        ...
        # Filter only GPT-2 models
        gpt2_configs = config_df[config_df['model_family'] == 'GPT-2']
        logger.info(f"Found {len(gpt2_configs)} GPT-2 configurations")
        ...
        missing_columns = [col for col in required_columns if col not in gpt2_configs.columns]
        if missing_columns:
    ...
    def scan_data_directory(self) -> List[Dict]:
        """Scan the multimodal dataset directory to find GPT-2 model directories and their feature files."""
        logger.info(f"Scanning multimodal dataset directory for GPT-2 models: {self.data_dir}")
        ...
        for model_dir in self.data_dir.rglob("*"):
            ...
            # Only process GPT-2 model directories
            if not self.is_gpt2_directory(model_dir):
                continue
            ...
            try:
                ...
                if model_info:
                    ...
                    logger.info(f"Found GPT-2 model directory: {model_dir} (Family: {family}, Index: {model_index})")
                else:
        ...
        logger.info(f"Found {dirs_with_features} GPT-2 directories with feature files")
        logger.info(f"Successfully processed {len(model_data)} GPT-2 model directories")
        ...
        if model_data:
            ...
            logger.info("Model family distribution from scanning (should be only GPT-2):")
            for family, count in family_counts.items():
                logger.info(f"  {family}: {count}")
        
        # Debug: List some directories that were found
        if total_dirs > 0 and dirs_with_features == 0:
            logger.warning("No GPT-2 directories with feature files found. Checking first few directories:")
            count = 0
            for model_dir in self.data_dir.rglob("*"):
    ...
    def is_gpt2_directory(self, model_dir: Path) -> bool:
        """Check if a directory contains GPT-2 models."""
        # Check if the directory path contains 'gpt-2' or 'gpt2'
        dir_path_str = str(model_dir).lower()
        return 'gpt-2' in dir_path_str or 'gpt2' in dir_path_str
    
    def extract_model_info_from_path(self, model_dir: Path) -> Optional[Dict]:
        """Extract model information from directory path for GPT-2 models only."""
        try:
            ...
            # Look for patterns like "108_gpt-2_small_adamw_lr1e-05_bs4"
            if '_' in dir_name:
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        # Only process GPT-2 models
                        if parts[1] == 'gpt-2' or parts[1] == 'gpt2':
                            model_family = 'GPT-2'
                        else:
                            # Skip non-GPT-2 models
                            return None
    ...
    def create_label_encodings(self, model_info: Dict) -> Dict:
        ...
        # Set model family label (should always be GPT-2)
        ...
        # For GPT-2 only, this should always be 1 at index 0
        if family == 'GPT-2':
            family_label[0] = 1  # GPT-2 is at index 0
            logger.debug(f"Set GPT-2 to index 0 in family_label: {family_label.tolist()}")
        else:
            logger.warning(f"Unexpected model family '{family}' - expected GPT-2")
        ...
        logger.debug(f"Final label encodings for GPT-2 model {model_info.get('model_index')}:")
    ...
    def match_with_config(self, model_data: List[Dict], config_df: pd.DataFrame) -> List[Dict]:
        """Match GPT-2 model data with config summary to fill missing information."""
        logger.info("Matching GPT-2 model data with config summary...")
        ...
        for model_info in model_data:
            ...
            logger.debug(f"Processing GPT-2 model {model_index} with initial family: {model_info.get('model_family')}")
            ...
            if model_index is not None:
                ...
                if not matching_configs.empty:
                    ...
                    logger.info(f"Matched GPT-2 model {model_index} with config: {old_family} -> {config_row['model_family']}")
                else:
                    logger.warning(f"No config found for GPT-2 model index {model_index}")
            ...
            if model_info.get('model_family') is None:
                for _, config_row in config_df.iterrows():
                    if model_dir in config_row['model_output_dir'] or config_row['model_output_dir'] in model_dir:
                        ...
                        logger.info(f"Matched GPT-2 model directory with config: {old_family} -> {config_row['model_family']}")
                        break
            
            # Log final model info
            logger.debug(f"Final GPT-2 model {model_index} family: {model_info.get('model_family')}")
    ...
    def validate_label_mappings(self):
        """Validate that label mappings are correct and consistent for GPT-2 only."""
        logger.info("Validating label mappings for GPT-2 models...")
        
        # Check model family mapping (should only be GPT-2)
        expected_families = ['GPT-2']
        if self.model_family_mapping != expected_families:
            ...
            logger.info("Updated model family mapping to expected values (GPT-2 only)")
        
        # Check model size mapping (GPT-2 sizes only)
        ...
        if self.model_size_mapping != expected_sizes:
            ...
            logger.info("Updated model size mapping to expected GPT-2 values")
        ...
        logger.info("Label mappings validation completed for GPT-2 models")
    ...
    def create_dataloader_csv(self) -> pd.DataFrame:
        """Create the dataloader.csv file with GPT-2 model information only."""
        logger.info("Creating dataloader.csv for GPT-2 models only...")
        
        # Validate label mappings first
        self.validate_label_mappings()
        
        # Load config summary (filtered for GPT-2 only)
        config_df = self.load_config_summary()
        
        # Scan multimodal dataset directory for GPT-2 models only
        ...
        logger.info("Model family distribution in matched data (should be only GPT-2):")
        for family, count in family_counts.items():
            logger.info(f"  {family}: {count}")
        ...
        logger.info(f"Created dataloader.csv with {len(dataloader_df)} GPT-2 entries")
    ...
    def print_summary_statistics(self, df: pd.DataFrame):
        """Print summary statistics of the created dataloader for GPT-2 models."""
        logger.info("\n" + "="*50)
        logger.info("DATALOADER SUMMARY STATISTICS (GPT-2 MODELS ONLY)")
        logger.info("="*50)
        ...
        if len(df) == 0:
            logger.warning("No GPT-2 data found! Please check:")
            logger.warning("1. The 'multimodal_dataset' directory exists and contains GPT-2 model directories")
            logger.warning("2. GPT-2 model directories contain feature files (x1_batch_*, x2_batch_*, etc.)")
            logger.warning("3. The config_summary.csv file contains GPT-2 configurations")
            logger.info("="*50)
            return
        
        # Only proceed with statistics if DataFrame has data
        if 'model_index' in df.columns:
            logger.info(f"Unique GPT-2 models: {df['model_index'].nunique()}")
        
        # Model family distribution (should only be GPT-2)
        ...
        # Model size distribution (GPT-2 sizes only)
        if 'model_size' in df.columns:
            logger.info("\nGPT-2 Model Size Distribution:")
...
def main():
    """Main function to create the dataloader for GPT-2 models only."""
    try:
        # Initialize creator
        creator = DataLoaderCreator()
        
        # Log expected model families (GPT-2 only)
        logger.info("Expected model families: GPT-2 only")
        logger.info("Expected model sizes: small, medium, large (GPT-2 only)")
        ...
        logger.info("Successfully created dataloader.csv and label_mappings.json for GPT-2 models only")
        logger.info(f"Files created in {creator.dataloader_dir}:")
        logger.info(f"  - dataloader.csv ({len(dataloader_df)} GPT-2 entries)")
        logger.info(f"  - label_mappings.json")
        
    except Exception as e:
        logger.error(f"Error creating dataloader for GPT-2 models: {str(e)}")
        raise
