"""
Script to generate and save features for trained shadow models.
Processes each trained model from the results directory and generates features.

This script:
1. Loads trained shadow models
2. Generates summaries for input texts
...
OUTPUT_DIR = "multimodal_dataset_exps1"  # Directory to save generated features
...
class ModelSummarizer:
    """Class to handle text summarization using a trained BART model."""
    
    def __init__(self, model_path: str):
        """
        Initialize the model summarizer with a trained model.
        
        Args:
            model_path (str): Path to the trained model directory
        """
        logger.info(f"Loading model from {model_path}")
        self.tokenizer = BartTokenizer.from_pretrained(model_path, cache_dir=os.environ['TRANSFORMERS_CACHE'])
        self.model = BartForConditionalGeneration.from_pretrained(model_path, cache_dir=os.environ['TRANSFORMERS_CACHE'])
    ...
    def summarize(self, text: str, max_length: int = 128, min_length: int = 30,
        ...
        """
        inputs = (
            "Summarize the following text as 3–5 short bullet points. "
            "Each bullet must start with '- ' and be on its own line.\n\n"
            "Text: " + text
        )
        inputs = self.tokenizer(inputs, max_length=1024, truncation=True, 
...
def process_model(df: pd.DataFrame, model_path: str, model_output_dir: str, num_batches: int):
    """
    Process all batches for a single model.
    ...
    summarizer = ModelSummarizer(model_path)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating summaries"):
...
def main():
    """Main function to process models and generate features."""
    ...
    parser = argparse.ArgumentParser(description='Process specific model indices')
    ...
    for _, row in config_df.iterrows():
        ...
        logger.info(f"\nProcessing model index {model_index}: {model_output_dir}")
        ...
        if check_features_exist(model_output_dir, num_batches):
            logger.info(f"Features already exist for model {model_output_dir}. Skipping...")
            continue
        
        try:
            process_model(df, model_path, model_output_dir, num_batches)
            logger.info(f"Completed processing for model {model_output_dir}")
        except Exception as e:
            logger.error(f"Error processing model {model_output_dir}: {str(e)}")
            continue
    
    logger.info(f"Processed all selected models. Features saved in directory: {OUTPUT_DIR}")
