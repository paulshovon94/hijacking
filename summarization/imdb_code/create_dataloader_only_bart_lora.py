"""
Data Loader Creator for BART-only LoRA models.
...
class LoRADataLoaderCreator:
    """Creates dataloader.csv for BART-only LoRA runs."""
    ...
    def __init__(
        ...
        # Label mappings aligned with BART LoRA policy.
    ...
    def load_config_summary(self) -> pd.DataFrame:
        ...
        config_df = config_df[config_df["model_family"] == "BART"].copy()
        logger.info("Filtered config to %d BART LoRA models", len(config_df))
        return config_df
    ...
    def match_with_config(self, model_data: List[Dict], config_df: pd.DataFrame) -> List[Dict]:
        ...
        for model_info in model_data:
            ...
            if model_info.get("model_family") != "BART":
    ...
    def validate_label_mappings(self) -> None:
        ...
        expected = {
            "model_family_mapping": ["BART"],
            "model_size_mapping": ["base", "large"],
...
def main():
    ...
    try:
        ...
        logger.info("Successfully created BART-only LoRA dataloader.csv and label_mappings.json")
