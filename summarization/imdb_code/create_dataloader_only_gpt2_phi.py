...
- Model index
- Model family (GPT-2, Phi)
- Model size (small, medium, large for GPT-2; 1.5 for Phi)
...
class DataLoaderCreator:
    ...
    Supports GPT-2 and Phi models with their respective configurations.
    """
    ...
    def __init__(self, data_dir: str = "./multimodal_dataset", config_path: str = "./configs/config_summary.csv"):
        ...
        # GPT-2 models: small (gpt2), medium (gpt2-medium), large (gpt2-large)
        # Phi models: 1.5 (phi-1.5)
        self.model_family_mapping = ['GPT-2', 'Phi']  # GPT-2 and Phi
        self.model_size_mapping = ['small', 'medium', 'large', '1.5']  # GPT-2 sizes + Phi size
    ...
    def load_config_summary(self) -> pd.DataFrame:
        ...
        # Filter GPT-2 and Phi models
        filtered_configs = config_df[config_df['model_family'].isin(['GPT-2', 'Phi'])]
        logger.info(f"Found {len(filtered_configs)} GPT-2 and Phi configurations")
        logger.info(f"  GPT-2: {len(filtered_configs[filtered_configs['model_family'] == 'GPT-2'])}")
        logger.info(f"  Phi: {len(filtered_configs[filtered_configs['model_family'] == 'Phi'])}")
    ...
    def scan_data_directory(self) -> List[Dict]:
        """Scan the multimodal dataset directory to find GPT-2 and Phi model directories and their feature files."""
        logger.info(f"Scanning multimodal dataset directory for GPT-2 and Phi models: {self.data_dir}")
        ...
        for model_dir in self.data_dir.rglob("*"):
            ...
            # Only process GPT-2 and Phi model directories
            if not self.is_gpt2_or_phi_directory(model_dir):
                continue
        ...
        logger.info(f"Found {dirs_with_features} GPT-2 and Phi directories with feature files")
        logger.info(f"Successfully processed {len(model_data)} GPT-2 and Phi model directories")
        ...
        if model_data:
            ...
            logger.info("Model family distribution from scanning (should be GPT-2 and Phi):")
            for family, count in family_counts.items():
                logger.info(f"  {family}: {count}")
        
        # Debug: List some directories that were found
        if total_dirs > 0 and dirs_with_features == 0:
            logger.warning("No GPT-2 or Phi directories with feature files found. Checking first few directories:")
            count = 0
            for model_dir in self.data_dir.rglob("*"):
    ...
    def is_gpt2_or_phi_directory(self, model_dir: Path) -> bool:
        """Check if a directory contains GPT-2 or Phi models."""
        # Check if the directory path contains 'gpt-2', 'gpt2', or 'phi'
        dir_path_str = str(model_dir).lower()
        return 'gpt-2' in dir_path_str or 'gpt2' in dir_path_str or 'phi' in dir_path_str
    
    def extract_model_info_from_path(self, model_dir: Path) -> Optional[Dict]:
        """Extract model information from directory path for GPT-2 and Phi models."""
        try:
            ...
            # Look for patterns like "108_gpt-2_small_adamw_lr1e-05_bs4" or "109_phi_1.5_adamw_lr1e-05_bs4"
            if '_' in dir_name:
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        if family_part == 'gpt-2' or family_part == 'gpt2':
                            ...
                            # Skip non-GPT-2 and non-Phi models
                            return None
    ...
    def create_label_encodings(self, model_info: Dict) -> Dict:
        ...
        # Set model family label (GPT-2 or Phi)
        ...
        logger.debug(f"Final label encodings for model {model_info.get('model_index')} ({family}):")
    ...
    def match_with_config(self, model_data: List[Dict], config_df: pd.DataFrame) -> List[Dict]:
        """Match GPT-2 and Phi model data with config summary to fill missing information."""
        logger.info("Matching GPT-2 and Phi model data with config summary...")
    ...
    def validate_label_mappings(self):
        """Validate that label mappings are correct and consistent for GPT-2 and Phi."""
        logger.info("Validating label mappings for GPT-2 and Phi models...")
        
        # Check model family mapping (GPT-2 and Phi)
        expected_families = ['GPT-2', 'Phi']
        if self.model_family_mapping != expected_families:
            ...
            logger.info("Updated model family mapping to expected values (GPT-2 and Phi)")
        
        # Check model size mapping (GPT-2 sizes + Phi size)
        expected_sizes = ['small', 'medium', 'large', '1.5']
        if self.model_size_mapping != expected_sizes:
            ...
            logger.info("Updated model size mapping to expected GPT-2 and Phi values")
        ...
        logger.info("Label mappings validation completed for GPT-2 and Phi models")
    ...
    def create_dataloader_csv(self) -> pd.DataFrame:
        """Create the dataloader.csv file with GPT-2 and Phi model information."""
        logger.info("Creating dataloader.csv for GPT-2 and Phi models...")
        
        # Validate label mappings first
        self.validate_label_mappings()
        
        # Load config summary (filtered for GPT-2 and Phi)
        config_df = self.load_config_summary()
        
        # Scan multimodal dataset directory for GPT-2 and Phi models
        model_data = self.scan_data_directory()
        
        # Match with config
        matched_data = self.match_with_config(model_data, config_df)
        
        # Filter to ensure only GPT-2 and Phi models remain
        matched_data = [
            model_info for model_info in matched_data 
            if model_info.get('model_family', '').upper() in ['GPT-2', 'PHI']
        ]
        logger.info(f"After filtering, {len(matched_data)} GPT-2 and Phi models remain")
        ...
        logger.info("Model family distribution in matched data (should be GPT-2 and Phi):")
        for family, count in family_counts.items():
            logger.info(f"  {family}: {count}")
        ...
        logger.info(f"Created dataloader.csv with {len(dataloader_df)} GPT-2 and Phi entries")
    ...
    def print_summary_statistics(self, df: pd.DataFrame):
        """Print summary statistics of the created dataloader for GPT-2 and Phi models."""
        logger.info("\n" + "="*50)
        logger.info("DATALOADER SUMMARY STATISTICS (GPT-2 AND PHI MODELS)")
        logger.info("="*50)
        ...
        if len(df) == 0:
            logger.warning("No GPT-2 or Phi data found! Please check:")
            logger.warning("1. The 'multimodal_dataset' directory exists and contains GPT-2 and Phi model directories")
            logger.warning("2. Model directories contain feature files (x1_batch_*, x2_batch_*, etc.)")
            logger.warning("3. The config_summary.csv file contains GPT-2 and Phi configurations")
            logger.info("="*50)
            return
        ...
        # Model family distribution (GPT-2 and Phi)
        ...
        # Model size distribution (GPT-2 and Phi sizes)
...
def main():
    """Main function to create the dataloader for GPT-2 and Phi models."""
    try:
        # Initialize creator
        creator = DataLoaderCreator()
        
        # Log expected model families (GPT-2 and Phi)
        logger.info("Expected model families: GPT-2 and Phi")
        logger.info("Expected model sizes: small, medium, large (GPT-2), 1.5 (Phi)")
        logger.info("GPT-2 models include: small (gpt2), medium (gpt2-medium), large (gpt2-large)")
        logger.info("Phi models include: 1.5 (phi-1.5)")
        ...
        logger.info("Successfully created dataloader.csv and label_mappings.json for GPT-2 and Phi models")
        logger.info(f"Files created in {creator.dataloader_dir}:")
        logger.info(f"  - dataloader.csv ({len(dataloader_df)} GPT-2 and Phi entries)")
        logger.info(f"  - label_mappings.json")
        
    except Exception as e:
        logger.error(f"Error creating dataloader for GPT-2 and Phi models: {str(e)}")
        raise
