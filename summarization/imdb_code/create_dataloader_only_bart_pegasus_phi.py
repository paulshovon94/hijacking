...
- Model index
- Model family (BART, Pegasus, Phi)
- Model size (base, large, xsum, 1.5, 2)
...
class DataLoaderCreator:
    ...
    Supports BART, Pegasus, and Phi models with their respective configurations.
    """
    ...
    def __init__(self, data_dir: str = "./multimodal_dataset", config_path: str = "./configs/config_summary.csv"):
        ...
        # Based on actual config_summary.csv, BART, Pegasus, and Phi models are processed
        # BART sizes: base, large
        # Pegasus sizes: large, xsum
        # Phi sizes: 1.5
        self.model_family_mapping = ['BART', 'Pegasus', 'Phi']
        self.model_size_mapping = ['base', 'large', 'xsum', '1.5']
    ...
    def extract_model_info_from_path(self, model_dir: Path) -> Optional[Dict]:
        ...
        try:
            ...
            # Look for patterns like "0_bart_base_adamw_lr1e-05_bs4" or "108_pegasus_xsum_adamw_lr1e-05_bs4"
            if '_' in dir_name:
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        # Handle different model family patterns (BART, Pegasus, and Phi)
                        model_family = parts[1].upper()
                        
                        # Skip GPT-2 models (but keep Phi)
                        if ('gpt' in model_family.lower() or model_family == 'GPT-2') and 'phi' not in model_family.lower():
                            return None
    ...
    def match_with_config(self, model_data: List[Dict], config_df: pd.DataFrame) -> List[Dict]:
        ...
        # Filter config_df to only include BART, Pegasus, and Phi models
        config_df = config_df[config_df['model_family'].isin(['BART', 'Pegasus', 'Phi'])]
        logger.info(f"Filtered config to {len(config_df)} BART, Pegasus, and Phi models")
        ...
        for model_info in model_data:
            ...
            # Skip GPT-2 models (but keep Phi)
            family = model_info.get('model_family', '')
            if ('gpt' in family.lower() or family == 'GPT-2') and 'phi' not in family.lower():
                logger.debug(f"Skipping GPT-2 model {model_index}")
                continue
    ...
    def validate_label_mappings(self):
        ...
        expected_families = ['BART', 'Pegasus', 'Phi']
        ...
        expected_sizes = ['base', 'large', 'xsum', '1.5']
    ...
    def create_dataloader_csv(self) -> pd.DataFrame:
        ...
        # Filter out any remaining GPT-2 models (but keep Phi)
        matched_data = [
            model_info for model_info in matched_data 
            if model_info.get('model_family', '').upper() in ['BART', 'PEGASUS', 'PHI']
        ]
        logger.info(f"After filtering, {len(matched_data)} BART, Pegasus, and Phi models remain")
...
def main():
    ...
    try:
        ...
        logger.info("Expected model families: BART, Pegasus, Phi")
        logger.info("Expected model sizes: base, large, xsum, 1.5")
        logger.info("BART sizes: base, large")
        logger.info("Pegasus sizes: large, xsum")
        logger.info("Phi sizes: 1.5")
