...
"""
Script to train Phi-1.5 shadow models using generated YAML configurations.
Specialized for Phi-1.5 models with specific configurations:
...
class SummarizationDataset:
    """Dataset class for handling summarization data for Phi-1.5 text generation."""
    ...
    def preprocess_function(self, examples: Dict) -> Dict:
        """Preprocess and tokenize input examples for Phi-1.5 text generation."""
        # For Phi-1.5, we'll format the input as: "Text: {input_text} Summary: {summary}"
...
def get_phi_model_and_tokenizer(config: Dict[str, Any]):
    """Get Phi-1.5 model and tokenizer based on configuration."""
...
def train_phi_model(config_path: str, model_index: int) -> None:
    """Train a Phi-1.5 model using the specified configuration."""
    ...
    logger.info(f"\nTraining Phi-1.5 model: {model_name}")
    ...
    try:
        ...
        # Get Phi-1.5 model and tokenizer
        model, tokenizer = get_phi_model_and_tokenizer(config)
        
        # Load datasets with Phi-1.5-specific configurations
        train_dataset = SummarizationDataset(
            config['data']['train_file'],
            tokenizer,
            config['data']['max_source_length']  # Should be 1024 for Phi-1.5
        )
        ...
        # Configure training arguments with Phi-1.5-specific settings
        training_args = TrainingArguments(
            ...
            fp16=config['training']['fp16'],  # Should be False for Phi-1.5 (A100: prefer bf16)
            bf16=config['training'].get('bf16', True),  # Should be True for Phi-1.5 (A100: better performance)
        ...
        data_collator = DataCollatorForLanguageModeling(
            tokenizer=tokenizer,
            mlm=False,  # Phi-1.5 is not a masked language model
        )
        ...
        logger.info("Starting Phi-1.5 training...")
        ...
        logger.info(f"Phi-1.5 training completed in {training_duration}")
        
        # Save model
        trainer.save_model(os.path.join(config['output']['output_dir'], "final_model"))
        
        # Evaluate on test set
        logger.info("Evaluating Phi-1.5 model on test set...")
        test_results = trainer.evaluate(test_hf)
        logger.info(f"Phi-1.5 test results: {test_results}")
        ...
        logger.error(f"Error training Phi-1.5 model {model_name}: {str(e)}")
        raise
    finally:
        if local_rank == 0:
            wandb.finish()
...
def main():
    """Main function to train Phi-1.5 shadow models."""
    import argparse
    import pandas as pd
    
    # Set up argument parser
    parser = argparse.ArgumentParser(description='Train Phi-1.5 shadow models')
    parser.add_argument('--model_indices', type=str, nargs='+', 
                      help='Indices of Phi-1.5 models to train. Can be individual indices (e.g., 1 2 3) or ranges (e.g., 0-9).')
    ...
    logger.info(f"Training {len(phi_df)} Phi-1.5 shadow models")
    
    # Train Phi-1.5 models
    for _, row in phi_df.iterrows():
        config_path = row['config_path']
        model_index = row['model_index']
        model_name = row['model_name']
        logger.info(f"\nProcessing Phi-1.5 model index {model_index}: {model_name}")
        logger.info(f"Config path: {config_path}")
        try:
            train_phi_model(config_path, model_index)
        except Exception as e:
            logger.error(f"Failed to train Phi-1.5 model with config {config_path}: {str(e)}")
            continue
