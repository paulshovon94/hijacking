...
class DataLoaderCreator:
    ...
    Supports BART and Pegasus models with their respective configurations.
    """
    
    def __init__(self, data_dir: str = "./multimodal_dataset_exps2", config_path: str = "./configs/config_summary.csv"):
        """
        ...
        # Based on actual config_summary.csv, only BART and Pegasus models are processed
            ...
            if '_' in dir_name:
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        # Handle different model family patterns (only BART and Pegasus)
                        model_family = parts[1].upper()
                        
                        # Skip GPT-2 models
                        if 'gpt' in model_family.lower() or model_family == 'GPT-2':
                            return None
    ...
    def match_with_config(self, model_data: List[Dict], config_df: pd.DataFrame) -> List[Dict]:
        ...
        # Filter config_df to only include BART and Pegasus models
        config_df = config_df[config_df['model_family'].isin(['BART', 'Pegasus'])]
        logger.info(f"Filtered config to {len(config_df)} BART and Pegasus models")
        ...
        for model_info in model_data:
            ...
            # Skip GPT-2 models
            family = model_info.get('model_family', '')
            if 'gpt' in family.lower() or family == 'GPT-2':
    ...
    def create_dataloader_csv(self) -> pd.DataFrame:
        ...
        # Filter out any remaining GPT-2 models
        matched_data = [
            model_info for model_info in matched_data 
            if model_info.get('model_family', '').upper() in ['BART', 'PEGASUS']
        ]
        logger.info(f"After filtering, {len(matched_data)} BART and Pegasus models remain")
...
def main():
    ...
    try:
        ...
        logger.info("Expected model families: BART, Pegasus")
        logger.info("Expected model sizes: base, large, xsum")
