...
"""
Feature extraction for Phi-2 LoRA shadow models.
...
`simple_shadow_models_phi_lora.py`.
"""
...
class PhiLoraTextGenerator:
    """Text generation wrapper for Phi-2 + LoRA adapters."""
    ...
    def __init__(self, model_output_dir: str):
        ...
        logger.info("Loaded Phi-2 LoRA model on %s", self.device)
...
def process_model(
    ...
    logger.info("Initializing Phi-2 LoRA generator and Sentence-BERT embedder...")
    text_generator = PhiLoraTextGenerator(model_output_dir_abs)
    ...
    for batch_num in range(1, num_batches + 1):
        ...
        transformed_texts = []
        generated_texts = []
        for _, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Generating LoRA Phi summaries"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create x1-x7 features for Phi-2 LoRA models.")
    parser.add_argument(
        "--model_indices",
        type=str,
        nargs="+",
    ...
    if not matched_rows:
        raise ValueError(f"No matching Phi-2 models found for indices: {selected_indices}")
