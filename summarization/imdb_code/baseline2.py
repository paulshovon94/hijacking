"""
Random Forest baseline for hyperparameter stealing.

This baseline mirrors the prediction task from experiment.py:
- Input: concatenated x1-x7 features (2312 dim)
- Outputs: model_family, model_size, optimizer, learning_rate, batch_size

Unlike the neural multi-head model, this script trains one RandomForestClassifier
per target head and reports per-head + overall metrics.
"""
...
def save_artifacts(
    ...
    models_path = output_dir / "random_forest_multitask_heads.pkl"
    ...
    metrics_path = output_dir / "random_forest_baseline2_metrics.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Random Forest baseline for multimodal hyperparameter prediction.")
    ...
    parser.add_argument("--output_dir", type=str, default="./models/random_forest_baseline2")
