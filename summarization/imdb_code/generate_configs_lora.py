...
class ConfigGeneratorLoRA:
    ...
    def __init__(self, output_dir: str = "./configs_lora"):
        self.output_dir = output_dir
        self.model_registry = ModelRegistry()

    @staticmethod
    def _get_family_data_settings(model_family: str) -> Dict[str, Any]:
        # BART + Pegasus (this generator): max_source_length=512, bf16, no fp16
        if model_family not in ("BART", "Pegasus"):
            logger.warning(
                "Unknown family %s in LoRA config; using 512 + bf16 defaults",
                model_family,
            )
        return {"max_source_length": 512, "fp16": False, "bf16": True}
    ...
    def create_config(self, model: ModelConfig, hp: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config = {
            ...
            "output": {
                "output_dir": f"./results/{model.family.lower()}/{model.size}/{run_name}",
    ...
    @staticmethod
    def get_model_hp_combinations(model: ModelConfig) -> List[Dict[str, Any]]:
        combinations = []
...
def main():
    try:
        generator = ConfigGeneratorLoRA()
        generator.generate_configs()

        logger.info("\nLoRA Configuration Generation Summary:")
        logger.info("Model families: BART, Pegasus")
