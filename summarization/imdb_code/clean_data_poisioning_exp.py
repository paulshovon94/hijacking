...
def load_features_and_labels_from_dataloader(dataloader_path: str, label_mappings_path: str, 
    ...
    if selected_model_indices is not None:
        if 'model_index' not in dataloader_df.columns:
            logger.warning("'model_index' column not found in dataloader. Cannot filter by model indices.")
        else:
            original_count = len(dataloader_df)
            dataloader_df = dataloader_df[dataloader_df['model_index'].isin(selected_model_indices)]
            logger.info(f"Filtered to {len(dataloader_df)} entries from {len(selected_model_indices)} selected model indices (original: {original_count})")
            if len(dataloader_df) == 0:
                raise ValueError(f"No entries found for the selected model indices: {selected_model_indices}")
...
def parse_model_indices(indices_str: str) -> List[int]:
    """
    Parse model indices from a string.
    ...
    Args:
        indices_str (str): String containing model indices
        
    Returns:
        List[int]: List of model indices
    """
    if not indices_str:
        return None
    ...
    for part in parts:
        ...
        if '-' in part:
            ...
            try:
                ...
                indices.extend(range(start_idx, end_idx + 1))  # Inclusive range
            except ValueError:
...
def main():
    ...
    parser.add_argument('--model_indices', type=str, default=None,
                       help='Comma-separated list of model indices to test (e.g., "0,1,2" or "0-5" for range)')
    ...
    if args.mode == 'inference':
        ...
        if args.model_indices:
            try:
                ...
                if rank == 0:
                    logger.info(f"Testing with model indices: {selected_model_indices}")
                    logger.info(f"Total {len(selected_model_indices)} model indices selected")
            except ValueError as e:
                if rank == 0:
                    logger.error(f"Error parsing model indices: {str(e)}")
                raise
        ...
        try:
            ...
            # Load features and labels from dataloader for evaluation (filtered by model indices if specified)
            features, labels, _ = load_features_and_labels_from_dataloader(
            ...
            if rank == 0:
                ...
                if args.output_path is None:
                    if selected_model_indices:
                        # Include model indices in filename
                        indices_str = "_".join(map(str, selected_model_indices[:10]))  # First 10 indices
                        if len(selected_model_indices) > 10:
                            indices_str += f"_and_{len(selected_model_indices)-10}_more"
                        output_path = os.path.join(MODELS_DIR, f"inference_results_model_indices_{indices_str}.csv")
                    else:
