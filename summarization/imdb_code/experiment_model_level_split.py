...
def load_features_and_labels_from_dataloader(dataloader_path: str, label_mappings_path: str) -> Tuple[np.ndarray, np.ndarray, Dict[str, List[str]], np.ndarray]:
    """
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray, Dict[str, List[str]], np.ndarray]: Features, labels, label mappings, and model indices
    """
    ...
    for idx, row in dataloader_df.iterrows():
        try:
            ...
            # Track model_index for this entry
            model_index = row['model_index']
            
            features_list.append(combined_features)
            labels_list.append(combined_labels)
            model_indices_list.append(model_index)
            processed_entries += 1
    ...
    # Convert model_indices to numpy array
    model_indices = np.array(model_indices_list)
    
    logger.info(f"\nSummary:")
    logger.info(f"Loaded {n_samples} samples")
    logger.info(f"Features shape: {features.shape}")
    logger.info(f"Labels shape: {labels.shape}")
    logger.info(f"Model indices shape: {model_indices.shape}")
    logger.info(f"Unique models: {len(np.unique(model_indices))}")
    logger.info(f"Processed {processed_entries} entries")
    logger.info(f"Skipped {skipped_entries} entries")
    
    return features, labels, label_mappings, model_indices
...
def main():
    ...
    try:
        ...
        features, labels, label_mappings, model_indices = load_features_and_labels_from_dataloader(DATALOADER_PATH, LABEL_MAPPINGS_PATH)
        ...
        # Model-level split: Group rows by shadow model ID
        # Get unique model indices from the processed entries
        unique_models = np.unique(model_indices)
        
        # Shuffle unique models using fixed seed for reproducibility
        rng = np.random.RandomState(args.seed)
        unique_models_shuffled = unique_models.copy()
        rng.shuffle(unique_models_shuffled)
        
        # Split unique models into train/val (80/20)
        train_size = int(0.8 * len(unique_models_shuffled))
        train_models = unique_models_shuffled[:train_size]
        val_models = unique_models_shuffled[train_size:]
        
        if rank == 0:
            logger.info(f"Total unique models: {len(unique_models_shuffled)}")
            logger.info(f"Training models: {len(train_models)}")
            logger.info(f"Validation models: {len(val_models)}")
        
        # Create masks for train/val split
        train_mask = np.isin(model_indices, train_models)
        val_mask = np.isin(model_indices, val_models)
        ...
        if rank == 0:
            ...
            logger.info(f"Train/Val split ensures no data leakage - all samples from a model are in the same set")
